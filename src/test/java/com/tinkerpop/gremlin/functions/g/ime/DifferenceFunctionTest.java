...
public class DifferenceFunctionTest extends BaseTest {
    ...
    public void testDifferenceInline() throws Exception {
        List results = evaluateGremlinScriptIterable("g:diff(g:list(1,1,2),g:list(1))", true);
        assertEquals(results.size(), 1);
        assertEquals(results.get(0), 2);

        results = evaluateGremlinScriptIterable("g:diff(g:list(1,1,2),g:diff(g:list(1,2,3),g:list(2,3)))", true);
        assertEquals(results.size(), 1);
        assertEquals(results.get(0), 2);
    }
