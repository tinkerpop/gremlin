...
public class IncludesFunctionTest extends BaseTest {
    ...
    public void testIncludesInline() throws Exception {
        GremlinScriptContext context = new GremlinScriptContext();
        Object result = evaluateGremlinScriptPrimitive("g:includes(g:list(1,2,3),1)", context, true);
        assertTrue((Boolean) result);

        result = evaluateGremlinScriptPrimitive("g:includes(g:list(g:list(1),2,3),1)", context, true);
        assertFalse((Boolean) result);

        List results = evaluateGremlinScriptIterable("g:list(1,1,2,1,1,2,2,2,2)[g:includes(g:list(1),.)]", context, true);
