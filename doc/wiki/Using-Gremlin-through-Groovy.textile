...
Note the use of @>>-1@. When creating a Gremlin expression, what is generated is an @Iterator@. When an @Iterator@ is returned in the Gremlin console (extends Groovy console), the @Iterator@ is iterated and the results are @==>@ printed. When the same expression is rendered in a Groovy class, the results are not iterated. As such, it is necessary to iterate the @Iterator@. In many cases, a @>>-1@ does the trick (see [[Gremlin Methods]]). @>>-1@ is equivalent to:
