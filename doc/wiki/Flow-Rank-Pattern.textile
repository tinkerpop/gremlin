...
```text
gremlin> software._().inE[[label:'created']].outV.name.groupCount.cap>>1
==>marko=1
==>peter=1
==>josh=2
```
...
```text
gremlin> m = [:]
gremlin> software._().inE[[label:'created']].outV.name.groupCount(m)
==>marko
==>josh
==>peter
==>josh
...
==>BIRD SONG=403
...
gremlin> println g.v(12).outE.inV.name.groupCount(m).back(2).loop{c++ < 1000}._{false}
[LoopPipe<[VertexEdgePipe<OUT_EDGES>, EdgeVertexPipe<IN_VERTEX>, FutureFilterPipe<[PropertyPipe<name>, GroupCountPipe]>]>, IdentityPipe, ClosureFilterPipe]
==>null
```
