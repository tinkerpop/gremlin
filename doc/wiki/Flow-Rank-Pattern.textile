...
==>0
gremlin> m = [:]                                                              
gremlin> g.v(12).out.name.groupCount(m).back(2).loop(2){c++ < 1000}.filter{false}
gremlin> m
...
==>BIRD SONG=403
...
gremlin> println g.v(12).out.name.groupCount(m).back(2).loop(2){c++ < 1000}.filter{false}
[LoopPipe<[OutPipe, BackFilterPipe[[PropertyPipe(name), GroupCountClosurePipe]]]>, ClosureFilterPipe]
==>null
```
