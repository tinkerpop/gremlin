...
```text
gremlin> software._().inE('created').outV.name.groupCount.cap>>1  
==>marko=1
==>peter=1
==>josh=2
```
...
```text
gremlin> m = [:]
gremlin> software._().inE('created').outV.name.groupCount(m)
==>marko
==>josh
==>peter
==>josh
...
[LoopPipe<[OutEdgesPipe, InVertexPipe, FutureFilterPipe<[PropertyPipe<name>, GroupCountClosurePipe]>]>, ClosureFilterPipe]
==>null
```
