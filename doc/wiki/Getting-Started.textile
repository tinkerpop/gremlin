...
```text
doskey gremlin=cd C:\Users\marko\software\gremlin$tgremlin.bat
```
...
Note that the default language for Gremlin is Groovy. It is possible to use (if provided) the REPL in another JVM language.

```
gremlin$ gremlin -l
The only Gremlin REPL JVM languages are 'groovy' and 'scala'
gremlin$ gremlin -l scala
...
```text
marko$ ./gremlin.sh -e example.groovy
1
2
3
marko$
