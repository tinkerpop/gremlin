...
==>29
gremlin> g.V.as('x').outE('knows').inV{it.age > 30}.back('x').age
==>29
```
...
gremlin> println g.v(89).outE('followed_by').inV.as('x').outE('sung_by').inV[[name:'Garcia']].back('x').name
[OutEdgesPipe(followed_by), AsPipe(x)[InVertexPipe], BackFilterPipe[[OutEdgesPipe(sung_by), InVertexPipe, PropertyFilterPipe(name,NOT_EQUAL,Garcia)]], PropertyPipe(name)]
==>null
```