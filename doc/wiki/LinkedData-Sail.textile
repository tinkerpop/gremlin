...
```text
gremlin> v.outE[[label:g.uri('foaf:made')]].inV       
...
```text
gremlin> v.outE[[label:g.uri('foaf:made')]].inV.outE[[label:g.uri('dcterms:title')]].inV.value
...
```text
gremlin> v.outE{labels.contains(it.label)}.inV.outE[[label:g.uri('foaf:name')]].inV.value                                   
==>Tom Constanten                                                           
==>Bill Kreutzmann
==>Phil Lesh
==>Grateful Dead
...
v.outE{labels.contains(it.label) & rand.nextBoolean()}.inV.groupCount(m).loop(4){it.loops < 4}



The results of the spreading activation are stored in the map referenced by @m@. Note that if you have a slow internet connection, this process may take some time. For this reason, a @rand.nextBoolean()@ "coin toss" is provided to half the amount of data processed. Please read about [[spreading activation]] to understand the algorithm in more detail. The call to @println it@ prints out each vertex traversed to allow the user to watch the walk happen in real-time. Please feel free to remove this without consequence to the computation. Finally, a sorted representation of the ranked results is presented below. The numeric value associated with each key in @m@ is the total flow through each vertex over the spreading activation process. Thus, the vertices with more energy flow are deemed more related to the Grateful Dead.

```text
gremlin> m.sort{ a,b -> b.value <=> a.value }
==>v[http://dbpedia.org/resource/Grateful_Dead]=27
==>v[http://dbpedia.org/resource/Tom_Constanten]=21
==>v[http://dbpedia.org/resource/Bob_Weir]=21
==>v[http://dbpedia.org/resource/Brent_Mydland]=19
==>v[http://dbpedia.org/resource/The_Other_Ones]=18
==>v[http://dbpedia.org/resource/Bill_Kreutzmann]=17
==>v[http://dbpedia.org/resource/Jerry_Garcia]=15
==>v[http://dbpedia.org/resource/Vince_Welnick]=15
==>v[http://dbpedia.org/resource/Keith_Godchaux]=14
...
```
